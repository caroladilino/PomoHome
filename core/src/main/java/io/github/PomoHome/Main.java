...
//    private ArrayList<Texture> textures;
    ...
    private final float POSICAO_LOJA_Y = 300f;
    ...
    public void create() {
        ...
        cash = Gdx.audio.newSound(Gdx.files.internal("cash.mp3"));
        ...
        if (Gdx.input.justTouched()) {
            ...
            for (int i = 0; i < estoque.size(); i++) {
                ...
                if (hitbox.contains(cliqueMouse)) {
                    ...
                    boolean sucesso = loja.comprarMovel(jogador, movelClicado);
                    if (sucesso) {
                        cash.play();
                    } else {
                        error.play();
    ...
    public void draw(){
        ...
        bitmap.draw(batch, "Loja:", POSICAO_INICIAL_X, POSICAO_LOJA_Y + 50f);
        ArrayList<Movel> estoque = loja.getMoveis();
        drawMoveis(estoque, POSICAO_INICIAL_X, POSICAO_LOJA_Y);

        bitmap.draw(batch, "Inventário:", POSICAO_INICIAL_X, POSICAO_INVENTARIO_Y + 50f);
        bitmap.draw(batch, ("Moedas: " + jogador.getSaldo()), POSICAO_INICIAL_X, POSICAO_INVENTARIO_Y + 25f);
        ArrayList<Movel> inventario = jogador.getMoveisAdquiridos();
